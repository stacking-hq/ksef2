...
- `ksef2.services.*`: implementacja workflow komponowana przez klienta; cztery
  udokumentowane typy usług importuj z `ksef2.clients` albo używaj modułów klienta;
...
<LinkCard
  ...
  description="Przejrzyj klientów głównych, klientów uwierzytelnionych i własność cyklu życia."
/>
<LinkCard
  title="API niskiego poziomu"
  href="low-level/overview.mdx"
  description="Użyj schema-native wrapperów endpointów przez wspieraną powierzchnię raw."
/>
<LinkCard
  title="Generowanie kodu sync"
  href="../contributing/sync-generation.md"
  description="Zobacz jak klient sync jest generowany z implementacji async."
/>
