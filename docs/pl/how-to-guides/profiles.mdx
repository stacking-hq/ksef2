---
title: Użyj profili
description: Skonfiguruj nazwane lokalne ustawienia uwierzytelniania w SDK.
---

import { Aside, LinkCard, Steps } from '@astrojs/starlight/components';

Profile to lokalne, nazwane ustawienia uwierzytelniania. Używaj ich, aby
korzystać z tego samego kontekstu KSeF w skryptach Pythona.
...
`with_profile()` czyta lokalny plik profili. Wywołanie bez nazwy używa
wybranego profilu.
...
wybierać albo sprawdzać lokalny plik profili. Użyj `with_profile()`,
...
2. Utwórz albo wybierz profil za pomocą `ProfileStore`.
