...
| `with_profile()` | Reads local profile configuration and dispatches to token, TEST certificate, PEM XAdES, or PKCS#12 XAdES authentication. |
...
| `with_profile()` | You want Python scripts to reuse named local authentication settings. | Resolves the selected profile and calls the matching SDK method. |
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Reuse named local authentication settings across Python scripts."
  />
</CardGrid>
...
The SDK selects profiles in this order:
