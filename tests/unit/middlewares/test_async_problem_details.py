...
class TestAsyncProblemDetailsMiddleware:
    @pytest.fixture
    def exceptions_middleware(self, async_fake_transport: AsyncFakeTransport):
        return middlewares.AsyncKSeFExceptionMiddleware(async_fake_transport)

    def test_400_problem_details(
        self,
        async_fake_transport: AsyncFakeTransport,
        exceptions_middleware: middlewares.AsyncKSeFExceptionMiddleware,
    ) -> None:
        body = {
            "title": "Bad Request",
            "status": 400,
        ...
        async_fake_transport.responses.append(_make_problem_response(400, body))

        with pytest.raises(exceptions.KSeFApiError) as exc_info:
            _ = asyncio.run(exceptions_middleware.request("POST", "/invoice"))
    ...
    def test_401_problem_details(
        ...
        async_fake_transport.responses.append(_make_problem_response(401, body))

        with pytest.raises(exceptions.KSeFAuthError) as exc_info:
            _ = asyncio.run(exceptions_middleware.request("GET", "/invoice"))
    ...
    def test_403_problem_details(
        ...
        async_fake_transport.responses.append(_make_problem_response(403, body))
    ...
    def test_410_problem_details(
        ...
        async_fake_transport.responses.append(_make_problem_response(410, body))

        with pytest.raises(exceptions.KSeFApiError) as exc_info:
            _ = asyncio.run(exceptions_middleware.request("GET", "/session"))
    ...
    def test_existing_exception_response_format_still_works(
        ...
        async_fake_transport.enqueue(status_code=400, json_body=body)
