...
def _field_info(annotation: object) -> FieldInfo:
    ...
    field_info = next(
        item for item in get_args(annotation)[1:] if isinstance(item, FieldInfo)
    )
    return field_info


def test_header_metadata_is_available_via_type_hints() -> None:
    hints = get_type_hints(StandardInvoiceBuilder.header, include_extras=True)
...
def test_body_metadata_is_available_for_billing_period() -> None:
    hints = get_type_hints(BaseBodyBuilder.billing_period, include_extras=True)
...
def test_add_line_metadata_marks_advanced_and_override_fields() -> None:
    hints = get_type_hints(RowsBuilder.add_line, include_extras=True)
...
def test_payment_metadata_is_available_for_partial_payment() -> None:
    hints = get_type_hints(PaymentBuilder.add_partial_payment, include_extras=True)
